...
export default function TimesheetsClient({ organisations }: { organisations: Org[] }) {
  ...
  useEffect(() => { setEmployeeId((prev) => employees.find((e: any) => e.id === prev) ? prev : (employees[0]?.id ?? "")); }, [orgId, employees]);
  ...
  return <div style={{ padding: 24 }}>
    ...
    <div style={panel}><div style={sectionTitle}>Selection</div><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
    ...
    <div style={panel}><div style={sectionTitle}>{tab === "OVERTIME" ? "Add overtime entry" : "Add part-time worked hours"}</div><div style={{ display: "grid", gridTemplateColumns: tab === "OVERTIME" ? "repeat(5, minmax(0, 1fr))" : "repeat(3, minmax(0, 1fr))", gap: 10 }}>
    ...
    </div><div style={{ marginTop: 10, display: "flex", gap: 10, alignItems: "center" }}><button style={primaryBtn} onClick={addEntry}>Save entry</button><div style={{ marginLeft: "auto", opacity: 0.85, fontWeight: 700 }}>{tab === "OVERTIME" ? `Total overtime: € ${money(total as number)}` : `Total hours: ${(Number(total) / 60).toFixed(2)}`}</div></div></div>

    <div style={panel}><div style={sectionTitle}>Entries</div>{rows.length === 0 ? <div style={{ opacity: 0.7 }}>No entries yet.</div> : <div style={{ display: "grid", gap: 10 }}>{rows.map((r) => <div key={r.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: 12, borderRadius: 12, border: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}><div style={{ minWidth: 110, fontWeight: 800 }}>{new Date(r.date).toISOString().slice(0, 10)}</div><div style={{ flex: 1, opacity: 0.85 }}>{(r.employee?.firstName ?? "")} {(r.employee?.lastName ?? "")} — {(Number(r.minutes ?? 0) / 60).toFixed(2)}h{r.notes ? ` — ${r.notes}` : ""}</div><div style={{ width: 140, textAlign: "right", fontWeight: 800 }}>{tab === "OVERTIME" ? `€ ${money(computeOvertimeCents(r))}` : `${(Number(r.minutes ?? 0) / 60).toFixed(2)} h`}</div><button style={dangerBtn} onClick={() => remove(r.id)}>Delete</button></div>)}</div>}</div>
  </div>;
}
