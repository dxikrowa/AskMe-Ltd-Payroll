...
export default function VacationLeaveClient({ organisations }: { organisations: Org[] }) {
  ...
  async function addEntry() {
    ...
    const res = await fetch("/api/vacation-leave", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ organisationId: orgId, employeeId, date: form.date, hours: form.hours, notes: form.notes }),
    });
    if (!res.ok) {
  ...
  async function addCarryForwardEntry(hours: string, notes: string) {
    if (!orgId || !employeeId) return;
    const res = await fetch("/api/vacation-leave-carry-forward", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ organisationId: orgId, employeeId, hours, notes }),
    });
    if (!res.ok) {
      alert((await res.json())?.error ?? "Failed");
      return;
  ...
  const entitledMinutes = employee ? annualEntitlementMinutes(employee) : 0;
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ opacity: 0.75, marginBottom: 14 }}>
        Record vacation leave used. Payslip generator will use entries within the pay period.
      </div>
      ...
      <div style={panel}>
        ...
        {employee ? (
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            <div><b>Used (this entitlement year):</b> {minutesToHours(usedMinutes)} h</div>
            <div><b>Remaining:</b> {minutesToHours(remainingMinutes)} h</div>
            {cycle ? (
              <div style={{ opacity: 0.75 }}>
                <b>Entitlement year:</b> {cycle.start.toISOString().slice(0, 10)} → {new Date(cycle.end.getTime() - 24 * 60 * 60 * 1000).toISOString().slice(0, 10)}
              </div>
            ) : null}
          </div>
        ) : (
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 14, flexWrap: "wrap" }}>
        <button
          style={{ ...ghostBtn, opacity: tab === "used" ? 1 : 0.75 }}
          onClick={() => setTab("used")}
        >
          Vacation leave used
        </button>
        <button
          style={{ ...ghostBtn, opacity: tab === "carry" ? 1 : 0.75 }}
      ...
      {tab === "used" ? (
        <>
          <div style={panel}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10 }}>
              ...
              <label>
                <div style={{ fontSize: 12, opacity: 0.7 }}>Hours used</div>
                <input value={form.hours} onChange={(e) => setForm((s) => ({ ...s, hours: e.target.value }))} style={input} placeholder="e.g. 8" />
              </label>
              <label>
        ...
        <CarryForwardTab
          carryRows={carryRows}
          onAdd={addCarryForwardEntry}
          onDelete={removeCarry}
        />
      )}
    </div>
  );
...
function CarryForwardTab({
  ...
  carryRows: any[];
  onAdd: (hours: string, notes: string) => Promise<void>;
    ...
    <>
      <div style={panel}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
          <label>
            <div style={{ fontSize: 12, opacity: 0.7 }}>Hours carried forward</div>
            <input value={hours} onChange={(e) => setHours(e.target.value)} style={input} placeholder="e.g. 12" />
          </label>
          <label>
            <div style={{ fontSize: 12, opacity: 0.7 }}>Notes</div>
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 10, alignItems: "center" }}>
          <button
            style={primaryBtn}
            onClick={async () => {
              await onAdd(hours, notes);
              setHours("");
              setNotes("");
            }}
          >
            Save carried forward
          </button>
        </div>
      </div>
      ...
      <div style={panel}>
        ...
        {carryRows.length === 0 ? (
          <div style={{ opacity: 0.7 }}>No carried forward entries yet.</div>
        ) : (
          <div style={{ display: "grid", gap: 10 }}>
            {carryRows.map((r) => (
              <div
              ...
              >
                <div style={{ minWidth: 160, fontWeight: 800 }}>
                  Entitlement start: {new Date(r.cycleStart).toISOString().slice(0, 10)}
                </div>
                <div style={{ flex: 1, opacity: 0.85 }}>
                  {(r.employee?.firstName ?? "")} {(r.employee?.lastName ?? "")}
                  {r.notes ? <span style={{ opacity: 0.7 }}> — {r.notes}</span> : null}
                ...
                <button style={dangerBtn} onClick={() => onDelete(r.id)}>
                  Delete
                </button>
              </div>
            ))}
