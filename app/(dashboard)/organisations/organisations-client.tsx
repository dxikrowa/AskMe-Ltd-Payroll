...
function taxStatusLabel(v: any): string {
  ...
  switch (n) {
    ...
    case 3:
      return "Parent";
    case 4:
      return "Married and 1 Child";
    case 5:
      return "Married and 2+ Children";
    case 6:
      return "Parent and 1 Child";
    case 7:
      return "Parent and 2+ Children";
    default:
      return "—";
  }
}
export default function OrganisationsClient({ organisations }: { organisations: Org[] }) {
  ...
  const filteredOrgs = useMemo(() => {
    ...
    return organisations.filter((o) => {
      const hay = `${o.name ?? ""} ${o.vatNumber ?? ""} ${o.phone ?? ""} ${o.peNumber ?? ""}`.toLowerCase();
  ...
  const selectedOrg = useMemo(() => {
    const pick = filteredOrgs.find((o) => o.id === selectedOrgId) ?? filteredOrgs[0] ?? null;
    return pick;
  ...
  const employees = useMemo(() => {
    ...
    return employeesAll.filter((e) => {
      const hay = `${e.firstName ?? ""} ${e.lastName ?? ""} ${e.email ?? ""} ${e.phone ?? ""} ${e.designation ?? ""} ${e.idNumber ?? ""}`.toLowerCase();
      return hay.includes(q);
    });
  ...
  // ---------- modal state ----------
  const [companyModal, setCompanyModal] = useState<{ mode: "add" | "edit"; open: boolean }>({
    mode: "add",
    open: false,
  });
  const [employeeModal, setEmployeeModal] = useState<{ mode: "add" | "edit"; open: boolean }>({
    mode: "add",
    open: false,
  });
  ...
  function openEditCompany() {
    ...
    setCompanyForm({
      ...
      addressHouseNo: selectedOrg.addressHouseNo ?? "",
      addressStreet: selectedOrg.addressStreet ?? "",
      addressLocality: selectedOrg.addressLocality ?? "",
      addressPostcode: selectedOrg.addressPostcode ?? "",
      vatNumber: selectedOrg.vatNumber ?? "",
      phone: selectedOrg.phone ?? "",
      phone2: selectedOrg.phone2 ?? "",
      peNumber: selectedOrg.peNumber ?? "",
      itRegistrationNumber: selectedOrg.itRegistrationNumber ?? "",
      jobsplusRegistrationNumber: selectedOrg.jobsplusRegistrationNumber ?? "",
      payrollManagerFullName: selectedOrg.payrollManagerFullName ?? "",
      payrollManagerPosition: selectedOrg.payrollManagerPosition ?? "",
    });
  ...
  function openEditEmployee() {
    ...
    setEmployeeForm({
      ...
      idNumber: selectedEmp.idNumber ?? "",
      spouseIdNumber: selectedEmp.spouseIdNumber ?? "",
      ssnNumber: selectedEmp.ssnNumber ?? "",
      addressHouseNo: selectedEmp.addressHouseNo ?? "",
      addressStreet: selectedEmp.addressStreet ?? "",
      addressLocality: selectedEmp.addressLocality ?? "",
      addressPostcode: selectedEmp.addressPostcode ?? "",
      baseWage: ((selectedEmp.baseWage ?? 0) / 100).toFixed(2),
      payFrequency: selectedEmp.payFrequency ?? "MONTHLY",
      taxStatus: selectedEmp.taxStatus ?? 1,
      employmentStartDate: (selectedEmp.employmentStartDate ? new Date(selectedEmp.employmentStartDate).toISOString().slice(0,10) : new Date().toISOString().slice(0,10)),
      normalWeeklyHours: String(selectedEmp.normalWeeklyHours ?? 40),
    ...
    startTransition(async () => {
      try {
        if (companyModal.mode === "add") {
          ...
          setCompanyModal({ ...companyModal, open: false });
          router.refresh();
          // pick new org after refresh (best-effort)
          if (res?.id) setSelectedOrgId(res.id);
        } else {
          if (!selectedOrg) return;
          await updateCompany({ id: selectedOrg.id, ...companyForm });
          setCompanyModal({ ...companyModal, open: false });
          router.refresh();
        }
      } catch (e: any) {
  ...
  async function onDeleteCompany() {
    ...
    startTransition(async () => {
      await deleteCompany({ id: selectedOrg.id });
  ...
  async function onSaveEmployee() {
    ...
    if (!employeeForm.firstName.trim() || !employeeForm.lastName.trim()) {
      return alert("Employee first name and last name are required.");
    }
      ...
      try {
        if (employeeModal.mode === "add") {
          const res = await createEmployee({
            ...
            ...employeeForm,
            taxStatus: Number(employeeForm.taxStatus),
          });
          setEmployeeModal({ ...employeeModal, open: false });
          router.refresh();
          if (res?.id) setSelectedEmpId(res.id);
        } else {
          if (!selectedEmp) return;
  ...
  async function onDeleteEmployee() {
    ...
    startTransition(async () => {
      await deleteEmployee({ id: selectedEmp.id });
      setSelectedEmpId(null);
      router.refresh();
    });
  }

  function importToCalculator() {
    if (!selectedEmp) return;
    router.push(`/payroll/run?employeeId=${selectedEmp.id}`);
  }
  ...
  return (
    <div style={{ display: "flex", gap: 12 }}>
      <div style={{ flex: 1, minWidth: 820 }}>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <input
            ...
            style={inputStyle}
            placeholder="Search company, VAT, phone, PE..."
          />
        </div>
        ...
        <div style={tableWrap}>
          ...
          <div style={tableBody}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderColor: active ? "rgba(59,130,246,0.35)" : "rgba(255,255,255,0.06)",
                  }}
                >
            ...
            {filteredOrgs.length === 0 ? (
              <div style={{ padding: 12, opacity: 0.7 }}>No companies match your search.</div>
            ) : null}
          </div>
        </div>

        {/* Selected company + employees (like your Python list under company row) */}
        {selectedOrg ? (
          <div style={{ marginTop: 12, display: "flex", gap: 12 }}>
            <div style={panel}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <div style={sectionTitle}>Company details</div>
                <button style={btnSmall} onClick={() => setShowCompanyDetails((v) => !v)}>
                  {showCompanyDetails ? "Hide" : "Show"}
                </button>
              </div>

              {showCompanyDetails ? (
                <div style={grid2}>
                  <Field k="Company">{selectedOrg.name ?? "—"}</Field>
                  <Field k="VAT">{selectedOrg.vatNumber ?? "—"}</Field>
                  <Field k="Phone">{selectedOrg.phone ?? "—"}</Field>
                  <Field k="Phone 2">{selectedOrg.phone2 ?? "—"}</Field>
                  <Field k="PE">{selectedOrg.peNumber ?? "—"}</Field>

                  <Field k="Address" clampLines={2}>
                    {[
                      selectedOrg.addressHouseNo,
                      selectedOrg.addressStreet,
                      `${selectedOrg.addressLocality ?? ""}${selectedOrg.addressPostcode ? `, ${selectedOrg.addressPostcode}` : ""}`.trim(),
                    ]
                      .filter(Boolean)
                      .join("\n") ||
                      [selectedOrg.address1, selectedOrg.address2, selectedOrg.city, selectedOrg.postcode]
                        .filter(Boolean)
                        .join("\n") ||
                      "—"}
                  </Field>

                  <Field k="Payroll manager">
            ...
            <div style={{ ...panel, flex: 1 }}>
              ...
              />
              {employees.length === 0 ? (
                <div style={{ opacity: 0.7 }}>No employees for this company.</div>
              ) : (
              ...
              {selectedEmp ? (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={grid2}>
                    ...
                    <Field k="Email">{selectedEmp.email ?? "—"}</Field>
                    <Field k="Phone">{selectedEmp.phone ?? "—"}</Field>
                    <Field k="ID Number">{selectedEmp.idNumber ?? "—"}</Field>
                    <Field k="SSN Number">{selectedEmp.ssnNumber ?? "—"}</Field>
                    <Field k="Gross Wage">
                      € {(((selectedEmp.baseWage ?? 0) / 100) as number).toFixed(2)}
                    </Field>
      ...
      {/* Right-side buttons column (exact like Python) */}
      <div style={rightCol}>
        ...
        <button style={btn} disabled={isPending || !selectedEmp} onClick={importToCalculator}>
          Run Payroll
        </button>
      </div>
      ...
      {companyModal.open ? (
        <Modal
          title={companyModal.mode === "add" ? "Add Company" : "Edit Company"}
          onClose={() => setCompanyModal({ ...companyModal, open: false })}
          onSave={onSaveCompany}
          disabled={isPending}
        >
          <FormGrid>
            <LabeledInput label="Company Name" value={companyForm.name} onChange={(v) => setCompanyForm((s) => ({ ...s, name: v }))} />
            {/* Address Line 1/2 removed (clutter). We keep legacy fields in DB for compatibility,
                and auto-compose them from the separated FSS address fields on save. */}
            <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>
              FSS address (recommended — fills House No / Street / Locality / Postcode lines)
            </div>
            ...
            <LabeledInput label="Phone Number 2" value={companyForm.phone2} onChange={(v) => setCompanyForm((s) => ({ ...s, phone2: v }))} />
      ...
      {employeeModal.open ? (
        <Modal
          title={employeeModal.mode === "add" ? "Add Employee" : "Edit Employee"}
          onClose={() => setEmployeeModal({ ...employeeModal, open: false })}
        ...
        >
          <FormGrid>
            ...
            <LabeledInput
              label="Spouse ID / Registration No. (if married)"
              value={employeeForm.spouseIdNumber}
              onChange={(v) => setEmployeeForm((s) => ({ ...s, spouseIdNumber: v }))}
            />
            ...
            <label style={{ display: "block" }}>
              ...
              <input
                type="date"
                value={employeeForm.employmentStartDate}
                onChange={(e) => setEmployeeForm((s) => ({ ...s, employmentStartDate: e.target.value }))}
                style={inputStyle}
              />
            </label>

            <LabeledInput
              label="Normal Weekly Hours (used for part-time pro-rata leave)"
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
              ...
              <div style={{ fontSize: 12, opacity: 0.8 }}>Under 18</div>
            </label>

            <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
              <input type="checkbox" checked={employeeForm.before1962} onChange={(e) => setEmployeeForm((s) => ({ ...s, before1962: e.target.checked }))} />
              <div style={{ fontSize: 12, opacity: 0.8 }}>Born up to 31 Dec 1961</div>
            </label>

            <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
              <input type="checkbox" checked={employeeForm.isStudent} onChange={(e) => setEmployeeForm((s) => ({ ...s, isStudent: e.target.checked }))} />
              <div style={{ fontSize: 12, opacity: 0.8 }}>Student</div>
            </label>


            <div style={{ fontSize: 12, opacity: 0.7, marginTop: 2 }}>Employee address (for FSS forms)</div>
            <LabeledInput label="House No." value={employeeForm.addressHouseNo} onChange={(v) => setEmployeeForm((s) => ({ ...s, addressHouseNo: v }))} />
            <LabeledInput label="Street" value={employeeForm.addressStreet} onChange={(v) => setEmployeeForm((s) => ({ ...s, addressStreet: v }))} />
            <LabeledInput label="Locality" value={employeeForm.addressLocality} onChange={(v) => setEmployeeForm((s) => ({ ...s, addressLocality: v }))} />
            <LabeledInput label="Postcode" value={employeeForm.addressPostcode} onChange={(v) => setEmployeeForm((s) => ({ ...s, addressPostcode: v }))} />

            <LabeledInput label="Gross Wage" value={employeeForm.baseWage} onChange={(v) => setEmployeeForm((s) => ({ ...s, baseWage: v }))} />
            ...
            <label style={{ display: "block" }}>
              ...
              <select
                value={String(employeeForm.taxStatus)}
                onChange={(e) => setEmployeeForm((s) => ({ ...s, taxStatus: Number(e.target.value) }))}
                style={inputStyle}
              >
                <option value="1">Individual</option>
                <option value="2">Married</option>
                <option value="3">Parent</option>
                <option value="4">Married with 1 Child</option>
                <option value="5">Married with 2+ Chuldren</option>
                <option value="6">Parent with 1 Child</option>
                <option value="7">Parent with 2+ Children</option>
              </select>
            </label>
          </FormGrid>
        </Modal>
    ...
    <div style={overlay} onMouseDown={onClose}>
      <div style={modal} onMouseDown={(e) => e.stopPropagation()}>
        ...
        <div style={modalFooter}>
          <button style={btnSmall} onClick={onSave} disabled={disabled}>Save</button>
          <button style={btnSmall} onClick={onClose}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
const empRow: React.CSSProperties = {
  padding: 12,
  borderRadius: 12,
  border: "1px solid var(--panel-border)",
  textAlign: "left",
