...
export async function POST(req: Request) {
  ...
  if (periodFrom && periodTo) {
    ...
    const defaultHourlyRateCents = Math.round(
      (employee.baseWage ?? 0) / MONTHLY_AVG_HOURS_FULLTIME
    );
  ...
  if (
    ...
    (pdfFields.basicpay_hours === "" || pdfFields.basicpay_hours == null)
  ) {
    pdfFields.basicpay_hours = (
      MONTHLY_AVG_HOURS_FULLTIME * ((employee.normalWeeklyHours ?? 40) / 40)
    ).toFixed(2);
  }
  ...
  if (
    (!pdfFields.hours_vlconsumed || String(pdfFields.hours_vlconsumed).trim() === "") &&
    periodFrom &&
    periodTo
  ) {
    ...
    if (typeof pdfFields.hours_vlconsumed !== "undefined") {
      pdfFields.hours_vlconsumed = minutesToHoursString(consumedMinutes);
    }
  }

  if (
    (!pdfFields.hours_vlremaining || String(pdfFields.hours_vlremaining).trim() === "") &&
    entitledMinutes
  ) {
    remainingMinutes = Math.max(0, entitledMinutes + carriedMinutes - consumedMinutes);

    if (typeof pdfFields.hours_vlremaining !== "undefined") {
      pdfFields.hours_vlremaining = minutesToHoursString(remainingMinutes);
    }
  }
  ...
  const baseForMaternityCents = Math.max(
    0,
    grossCents - overtimeCents - bonusCents - allowanceCents
  );
